...
def mouse_callback(event, x, y, flags, param):
    global is_drawing_on_frame
    global is_paused
    global annotated_frame

    if event == cv2.EVENT_LBUTTONDOWN and is_paused:
        print("mousedown event")
        is_drawing_on_frame = True
        wip_detection_area.append((x, y))
        if len(wip_detection_area) == 4:
            cv2.polylines(
                annotated_frame,
                [np.array(wip_detection_area, np.int32)],
                True,
                (255, 0, 255),
                4,
            )
            cv2.imshow("YOLOv8 Tracking", annotated_frame)
            detection_areas.append(wip_detection_area.copy())
            wip_detection_area.clear()
            is_drawing_on_frame = False

    if event == cv2.EVENT_MOUSEMOVE and is_drawing_on_frame:
        print("mousemove event")
        temp_area = wip_detection_area.copy()
        temp_area.append((x, y))
        temp_annotated_frame = annotated_frame.copy()
        cv2.polylines(
...
while capture.isOpened():
    is_paused = False
    wip_detection_area = []  # cleanup partial polygon
    ...
    if results[0].boxes.id is not None:
        ...
        for x1, y1, x2, y2, track_id, score, vehicle_type_id in results[0].boxes.data.numpy():
            ...
            cv2.circle(annotated_frame, (int(x2), int(y2)), 6, (0, 255, 70), -1)
    ...
    for area in detection_areas:
        cv2.polylines(annotated_frame, [np.array(area, np.int32)], True, (255, 0, 255), 4)
